...
assay(vsd) <- limma::removeBatchEffect(assay(vsd),vsd$lib_prep_batch)

# Visualize using PCA
data <- plotPCA(vsd, intgroup=c("pathology","sex","age","lib_prep_batch"), returnData=TRUE)
percentVar <- round(100 * attr(data, "percentVar"))

plot <- ggplot(data, aes(PC1, PC2, ID, color=pathology, fill=pathology, shape=sex)) +
  scale_shape_manual(values=c(22,24), 
                     name="Sex") +
  geom_point(size=3) +
 # geom_text(aes(label=name), size=3, hjust=0.5, vjust=1.5) +
  scale_fill_manual(values=mycols, 
                    labels=c("NAAP","PA","SA"),
                    name="Pathology") +
  ...
  theme_bw()
plot
ggsave("../results/PCA_corrected_lib_prep_batch_2022.png")
...
```{r pcatools, eval=FALSE}

p <- pca(vsd, metadata = colData(ddsHTSeq))
...
rownames(mat) <- colnames(mat) <- with(colData(ddsHTSeq), paste(sex))
